...
function App() {
  ...
  const refresh = React.useCallback(() => {
    AtlantisData.load()
      .then(({ products, at, live, fallback, count }) => setState({ status: 'ready', products, meta: { at, live, fallback, count: products.length }, error: null }))
      .catch((e) => setState((s) => ({ status: s.products ? 'ready' : 'error', products: s.products, meta: s.meta, error: String(e.message || e) })));
  }, []);
  ...
  const metaStr = `${m.fallback ? 'Beispieldaten' : m.live ? 'Live' : 'Beispieldaten'} · ${m.count.toLocaleString('de-DE')} Artikel · Stand ${stamp}`;
