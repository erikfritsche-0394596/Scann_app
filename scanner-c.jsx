...
function ScannerC({ tw, products }) {
  ...
  const open = (p) => { setDetail(p); setHistory((h) => [{ p, at: Date.now() }, ...h.filter((x) => x.p.ean !== p.ean)].slice(0, 20)); };
  const startScan = () => {
    if (scanning) return;
    setScanning(true);
    setTimeout(() => {
      const p = PRODUCTS[idx.current % PRODUCTS.length];
      idx.current += 1;
      setScanning(false);
      open(p);
    }, 1400);
      ...
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: T.bg }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', WebkitOverflowScrolling: 'touch', padding: T.pad }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: T.gap, marginTop: T.gap + 4 }}>
            ...
            <Tile>
              <TileLabel icon={Icon.box}>Bestand · {detail.home || 'Coppi'}</TileLabel>
              <div style={{ fontSize: F(24), fontWeight: 800, color: T.stock[st], marginTop: 6, lineHeight: 1 }}>{detail.stock}<span style={{ fontSize: F(13), fontWeight: 600, color: T.mute }}> Stk</span></div>
              <div style={{ marginTop: 5, fontSize: F(11), color: T.mute }}>vor Ort · {detail.stockTotal} gesamt</div>
            ...
            <div style={{ background: T.card, borderRadius: T.radius, padding: T.pad, marginTop: T.gap, border: `1px solid ${T.border}`, boxShadow: T.tileShadow }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(60px, 1fr))', gap: 8, marginTop: 10 }}>
                {detail.variants.map((vr) => {
                  ...
                  return (
                    <div key={vr.v} style={{ borderRadius: 10, padding: '7px 4px', textAlign: 'center', background: bg, border: `1px solid ${T.border}` }}>
          ...
          <div style={{ marginTop: 10, marginBottom: 6, display: 'flex', justifyContent: 'space-between', fontFamily: 'ui-monospace, Menlo, monospace', fontSize: F(11), color: T.mute }}>
            <span>Art. {detail.art}</span><span>EAN {detail.ean}</span>
          </div>
        </div>
  ...
  const matches = q2.length >= 2
    ? PRODUCTS.filter((p) => (p._s || (p.name + ' ' + p.brand + ' ' + p.art + ' ' + p.cat + ' ' + (p.allEans || []).join(' ')).toLowerCase()).includes(q2))
    : [];
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: T.bg }}>
      ...
      <div style={{ padding: `12px ${T.pad}px 0` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: T.field, borderRadius: 12, padding: '10px 14px', border: `1px solid ${T.border}`, boxShadow: T.tileShadow }}>
          ...
          {q && <button onClick={() => setQ('')} style={{ border: 'none', background: 'none', cursor: 'pointer', padding: 0, display: 'flex' }}>{Icon.close(T.mute, 18)}</button>}
        </div>
      </div>
      <div style={{ flex: 1, overflow: 'auto', padding: T.pad, display: 'flex', flexDirection: 'column', gap: T.gap - 2 }}>
        {q2.length < 2 ? (
          <div style={{ textAlign: 'center', color: T.mute, marginTop: 50, fontSize: F(14), lineHeight: 1.5 }}>Mindestens 2 Zeichen eingeben<br />— Name, Marke, Art.-Nr. oder EAN</div>
        ) : shown.length ? (
          <>
